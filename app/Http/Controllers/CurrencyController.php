...
use App\Http\Requests\Currency\AddRequest;
...
class CurrencyController extends Controller
{
    ...
    {
      $results = Currency::with(['createdBy', 'updatedBy'])->get();
      // dd($dishes->toArray());
    ...
    public function store(AddRequest $request)
    {
      ...
      try {
        if ($validated['default'] == 'yes') {
          Currency::where('default', 'yes')->update([
           'default' => 'no',
           'updated_by' => Auth::id()
          ]);
        }
        Currency::createWithTransaction($validated);
    ...
    public function update(AddRequest $request, string $id)
    {
      ...
      try {
        ...
        Currency::updateWithTransaction($id, $validated);
    ...
    {
      try {
        Currency::deleteWithTransaction($id);
