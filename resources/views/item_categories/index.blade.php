                            ...
                            <tbody>
                                @foreach($results as $key => $result)
                                    <tr>
                                        ...
                                        <a title="Edit" href="{{ route($edit, ['item_category' => $result->id]) }}" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm me-3">
                                          <i class="fa-regular fa-pen-to-square"></i>
                                        </a>
                                        @endif
                                        @if(hasPermissionForModule('delete', $currentModuleId))
                                        <!-- Delete Form (Hidden) -->
                                        <form id="delete-form-{{ $result->id }}" action="{{ route($destroy, ['item_category' => $result->id]) }}" method="POST" style="display: none;">
                                          @csrf
                                          @method('DELETE')
                                        </form>
