          ...
          <div class="row">
            <div class="col-md-4 col-lg-3 mb-5">
              ...
              <select class="form-select " name="item_id" data-control="select2" data-close-on-select="false" data-placeholder="Select" data-allow-clear="true">
                ...
                <option value="{{ $item->id }}" {{ old('item_id',$result->item_id) == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                @endforeach
              </select>
            ...
            <div class="col-md-4 col-lg-3 mb-5">
              ...
              <select class="form-select " id="unit_measure_id" name="unit_measure_id" data-control="select2" data-close-on-select="false" data-placeholder="Select" data-allow-clear="true">
                ...
                <option value="{{ $unitItem->id }}" {{ old('unit_measure_id',$result->unit_measure_id) == $unitItem->id ? 'selected' : '' }}>{{ $unitItem->name }}</option>
                @endforeach
              </select>
...
<script>
  ...
  $(document).on("change", "#unit_measure_id", function() {
        ...
        @foreach($unitMeasure as $unit)
            ${baseUomSelect != "{{ $unit->id }}" ? 
                `<option value="{{ $unit->id }}" {{ in_array($unit->id, $result->unitMeasure->pluck('id')->toArray()) || old('unit_measure_id') == $unit->id ? 'selected' : '' }}>{{ $unit->name }}</option>` 
                : ''
            }
        @endforeach
    `;
