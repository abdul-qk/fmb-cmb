                ...
                <div class="">
                    <div class="card">
                      ...
                      <div class="event-detail" @if(count(request()->query()) <= 1) style="display: none;" @endif>
                        <form class="form mb-0" method="GET" action="{{ route($indexRoute) }}"
                            style="padding:1rem 2.25rem 0">
                            @csrf
                            <div class="row">
                                <div class="col-md-2 mb-5">
                                    <label class="form-label">Event Date From</label>
                                    <input type="date" class="form-control" name="date_from" id="date_from"
                                        value="{{ request('date_from') ? request('date_from') : $dateFrom }}">
                                </div>
                                <div class="col-md-2 mb-5">
                                    <label class="form-label">Event Date To</label>
                                    <input type="date" class="form-control" name="date_to" id="date_to"
                                        value="{{ request('date_to') ? request('date_to') : $dateTo }}">
                                </div>
                                <div class="col-md-2 mb-5">
                                    <label class="form-label">Event</label>
                                    <select class="form-select" name="event_id" data-control="select2"
                                        data-close-on-select="false" data-placeholder="Select" data-allow-clear="true">
                                        <option value=""></option>
                                ...
                                <div class="col-md-2 mb-5">
                                    ...
                                    <select class="form-select" name="item_category_id" data-control="select2"
                                        data-close-on-select="false" data-placeholder="Select" data-allow-clear="true">
                                        <option value=""></option>
                                        @foreach ($itemCategories as $itemCategory)
                                            <option value="{{ $itemCategory->id }}"
                                                {{ request('item_category_id') == $itemCategory->id ? 'selected' : '' }}>
                                ...
                                <div class="col-md-2 mb-5">
                                    ...
                                    <select class="form-select" name="item_id" data-control="select2"
                                        data-close-on-select="false" data-placeholder="Select" data-allow-clear="true">
                                        <option value=""></option>
                        ...
                        <div class="card-body py-0">
                            <div class="table-responsive">
                                @php
                                    // Collect all unique event name + serving labels
                                    $eventLabels = collect($results)
                                        ->flatMap(function ($result) {
                                            $events = $result['requested_events'] ?? [];
                                            $servings = $result['requested_event_servings'] ?? [];

                                            return collect($events)->map(function ($event, $i) use ($servings) {
                                                $eventName = is_object($event) ? $event->name ?? null : $event;
                                                $serving = $servings[$i] ?? null;
                                                return $eventName && $serving ? "{$eventName}<br>" . ucfirst($serving) : $eventName ?? null;
                                            });
                                        })
                                        ->filter()
                                        ->unique()
                                        ->sortBy(function ($label) {
                                            if (preg_match('/\b(\d{1,2})(st|nd|rd|th)?\s+(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+\d{4}\b/i', $label, $matches)) {
                                                return \Carbon\Carbon::parse($matches[0]);
                                            }
                                            if (preg_match('/\b(\d{1,2})(st|nd|rd|th)?\s+(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\b/i', $label, $matches)) {
                                                try {
                                                    $dayMonth = $matches[1] . ' ' . $matches[3];
                                                    $year = now()->year;
                                                    return \Carbon\Carbon::parse("{$dayMonth} {$year}");
                                                } catch (\Exception $e) {
                                ...
                                <table id="kt_datatable" class="table table-bordered gy-5">
                                    ...
                                    <tbody>
                                        @foreach ($results as $result)
                                            @php
                                                $inHand = ($result['inventory_quantity'] - $result['issued_quantity'] - $result['supplier_returned_quantity']) + $result['returned_quantity'];
                                                $shortage = $result['requested_quantity'] - $inHand;

                                                $events = $result['requested_events'] ?? [];
                                                $quantities = $result['requested_quantities'] ?? [];
                                                $servings = $result['requested_event_servings'] ?? [];
                                                ...
                                                foreach ($events as $i => $event) {
                                                    ...
                                                    $eventName = is_object($event) ? $event->name ?? null : $event;
                                                    $eventLabel = $eventName && $serving ? "{$eventName}<br>" . ucfirst($serving) : $eventName ?? null;

                                                    if ($eventLabel && trim($eventLabel) !== '') {
                                                        $eventData[$eventLabel] = ($eventData[$eventLabel] ?? 0) + $quantity;
                                                    } else {
                                                        $grnTotal += $quantity;
                                                    }
                                                }
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ $grnTotal > 0 ? $result['requested_quantity'] - $grnTotal : $result['requested_quantity'] }}</td>
                                                <td>{{ $inHand }}</td>
                                                <td>
                                                    <span class="{{ $shortage > 0 ? 'text-danger opacity-75' : '' }}">
                                                        {{ $shortage }}
                                                    </span>
                                                </td>
                                                <td>{{ $result['last_purchased_price'] }}</td>
                                                @foreach ($eventLabels as $label)
            ...
            var table = $("#kt_datatable").DataTable({
                ...
                "buttons": [{
                        ...
                        filename: function() {
                            ...
                            return `FMB_Procurement_Requisition_Report_${dateFrom}_${dateTo}`;
                        }
                    },
                    {
                        extend: 'pdfHtml5',
