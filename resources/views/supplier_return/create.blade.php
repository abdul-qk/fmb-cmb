@php
  $fetchedItems = session('supplier_return_items', []);
@endphp
        ...
        <form class="form" method="POST" action="{{ route($store) }}">
          ...
          <div class="row">
            <div class="col-md-3 col-lg-3 mb-5">
              ...
              <select class="form-select" id="grn_id" name="grn_id" data-control="select2" data-close-on-select="false" data-placeholder="Select" data-allow-clear="true">
                <option value=""></option>
                @foreach($grns as $grn)
                <option value="{{ $grn->id }}" {{ old('grn_id') == $grn->id ? 'selected' : '' }}>#{{ $grn->id }}_{{ $grn->grn_date }}_{{ $grn->bill_no }}</option>
                @endforeach
              </select>
              @if ($errors->has('grn_id'))
              <span class="text-danger">{{ $errors->first('grn_id') }}</span>
              @endif
            </div>
            <div class="col-md-3 col-lg-3 mb-5">
              <label class="form-label">Vendor</label>
              <input type="text" class="form-control" id="vendor" name="vendor" value="{{ old('vendor', null) }}" readonly/>
              <input type="hidden" class="form-control" id="vendor_id" name="vendor_id" value="{{ old('vendor_id', null) }}"/>
            </div>
          </div>

          <div id="items-section" style="display: {{ old('grn_id') ? 'block' : 'none' }};">
                  ...
                  <tr id="item-row-{{ $index }}">
                      <td>
                          <select class="form-select item-select" name="items[{{ $index }}][item_id]" data-control="select2" data-close-on-select="false" data-placeholder="Select" data-allow-clear="true">
                              <option value=""></option>
                              @foreach($fetchedItems as $availableItem)
                      ...
                      <td>
                          <input type="number" step="0.001" class="form-control available-quantity w-150px w-lg-100" name="items[{{ $index }}][available_quantity]" value="{{$item['available_quantity']}}" readonly>
                      </td>
                      <td>
...
<script>
  $(document).ready(function() {
    ...
    var fetchedUnits = [];
        ...
        // Show items section if gnr is selected
    $('#grn_id').on("change", function(event) {
        event.preventDefault();
        gnrId = this.value;
        $("#items-section").css("display", gnrId ? "block" : "none");
        $('#items-table-body').empty();

        if (gnrId) {
            $.ajax({
                type: 'GET',
                url: "{{ route('fetchGrnItems') }}",
                data: { grn_id: gnrId },
                success: function(response) {
                    if (response?.items) {
                      fetchedItems = response.items;
                      fetchedUnits = response.units;
                      $('#vendor').val(response?.vendor?.name);
                      $('#vendor_id').val(response?.vendor?.id);
                      totalItems = Object.values(fetchedItems).length;
    ...
    function addItemRow(items, units,itemValues) {
        ...
        $('#items-table-body').append(`
            <tr id="item-row-${itemIndex}">
                ...
                <td>
                    <input type="text" step="0.001" class="form-control available-quantity w-150px w-lg-100" name="items[${itemIndex}][available_quantity]" readonly>
                </td>
                <td>
                ...
                <td class="text-center" style="vertical-align: middle;">
                  <svg id="add-item-btn" xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" width="25" height="25" x="0" y="0" viewBox="0 0 24 24" style="enable-background:new 0 0 512 512" xml:space="preserve" class="cursor-pointer add-item-btn"><g><path d="M18 2c2.206 0 4 1.794 4 4v12c0 2.206-1.794 4-4 4H6c-2.206 0-4-1.794-4-4V6c0-2.206 1.794-4 4-4zm0-2H6a6 6 0 0 0-6 6v12a6 6 0 0 0 6 6h12a6 6 0 0 0 6-6V6a6 6 0 0 0-6-6z" fill="var(--bs-primary)" opacity="1" data-original="#000000" class=""></path><path d="M12 18a1 1 0 0 1-1-1V7a1 1 0 0 1 2 0v10a1 1 0 0 1-1 1z" fill="var(--bs-primary)" opacity="1" data-original="#000000" class=""></path><path d="M6 12a1 1 0 0 1 1-1h10a1 1 0 0 1 0 2H7a1 1 0 0 1-1-1z" fill="var(--bs-primary)" opacity="1" data-original="#000000" class=""></path></g></svg>
                      
                  <svg title="Remove" class="ms-3 remove-item-btn" data-row-id="item-row-${itemIndex}" xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" width="25" height="25" x="0" y="0" viewBox="0 0 384 384" style="enable-background:new 0 0 512 512" xml:space="preserve"><g><path d="M64 341.333C64 364.907 83.093 384 106.667 384h170.667C300.907 384 320 364.907 320 341.333v-256H64v256zM266.667 21.333 245.333 0H138.667l-21.334 21.333H42.667V64h298.666V21.333z" fill="#f42c2c" opacity="1" data-original="#000000" class=""></path></g></svg>
                </td>
            </tr>
        `);
    ...
    $('#items-table-body').on('click', '.remove-item-btn', function(event) {
        ...
        if ($('#items-table-body tr').length === 0) {
          $("#items-section").css("display", "none");
          $('#grn_id').val('').trigger('change'); // Optionally reset gnr selection
        }
    });
      ...
      if (item != null && $(this).val() != null) {
        ...
        $.get("/fetch-uom-base", data, function(response) {
          if(response.success == false) {
            ...
            alert(`The UOM conversion for ${response?.itemBaseUom?.base_uom?.short_form} to ${$(loader).find("option:selected").text()} is not set.`)
          }else {
    ...
    function populateItemDetails(loader, itemId) {
      ...
      let availableQuantity = $(loader).closest("tr").find(".available-quantity");
      let data = {
        id: itemId,
      };
      $.get("/fetch-item-details", data, function(response) {
        $(baseUom).val(response?.item_base?.base_uom?.short_form);
        $(availableQuantity).val(response?.detail?.available_quantity);
      });
    }
  });
</script>
