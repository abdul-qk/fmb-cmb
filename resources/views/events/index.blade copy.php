          ...
          <div class="card-toolbar">
            ...
            <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
              ...
              <a type="button" class="btn btn-primary me-2" href="{{ route($create) }}">
                <i class="ki-outline ki-plus fs-2"></i>Add {{ $moduleNameSingular }}
              </a>
              <a type="button" class="btn btn-primary " href="/events/monthly/create">
                <i class="ki-outline ki-plus fs-2"></i>Add Monthly Event
              </a>
              @endif
              <!--end::Add button-->
            </div>
            ...
            <table id="kt_datatable" class="table table-bordered gy-5">
              <thead>
                <tr class="fw-semibold fs-6 text-muted">
                  ...
                  <th style="vertical-align: middle;">Serving</th>
                  <th style="vertical-align: middle;">Qty </th>
                  <th style="vertical-align: middle;">Status</th>
                  <!-- <th>Description</th> -->
                  <th style="vertical-align: middle;">Action</th>
                  <th style="vertical-align: middle;">Created</th>
                  <th style="vertical-align: middle;">Last Modified</th>
                </tr>
              </thead>
              <tbody>
                ...
                <tr class="text-capitalize">
                  ...
                  <td>
                    {{ $result["event"]["date"] ? \Carbon\Carbon::parse($result["event"]["date"])->isoFormat('Do MMM YYYY') : "-" }}
                    <br />
                    {{ ($result["event"]["start"] && $result["event"]["end"]) ? \Carbon\Carbon::parse($result["event"]["start"])->isoFormat('hh:mm A') .' - '. \Carbon\Carbon::parse($result["event"]["end"])->isoFormat('hh:mm A') : '-' }}
                    <br />
                    @php
                    // Split the time string into hours and minutes
                    $timeParts = explode(':', $result["event"]["event_hours"]);
                    $hours = isset($timeParts[0]) ? (int)$timeParts[0] : 0;
                    $minutes = isset($timeParts[1]) ? (int)$timeParts[1] : 0;

                    // If there are no hours and no minutes, set a flag for display
                    ...
                    @endif
                    @if ($minutes > 0)
                    {{ ($hours > 0 ? ' and ' : '') . $minutes . ' ' . \Illuminate\Support\Str::plural('Minute', $minutes) }}
                    @endif
                    @else
                    {{ '-' }}
                    @endif
                  ...
                  <td>{{$result["event"]["meal"]}}</td>
                  <td>{{$result["event"]["serving"]}}</td>
                  <td>
                    {!! $result['event']['no_of_thaal'] ? 'Thaal: ' . $result['event']['no_of_thaal'] . '<br />' : '' !!}
                    @if($result['serving_items']->isNotEmpty())
                    @foreach($result['serving_items'] as $servingItem)
                    <p>{{ ucfirst($servingItem['tiffin_size_name']) }} Tiffin ({{ $servingItem['tiffin_size_person_no'] }} pax): {{ $servingItem['count'] }}</p>
                    @endforeach
                    @endif
                  </td>
                  <td> {{$result["event"]["status"]}} </td>
                  <!-- <td>{{$result["event"]["description"] ? $result["event"]["description"] : '-' }}</td> -->
                  <td>
                    @if($result["event"]["status"] == 'Pending')
                    <form id="approved-form-{{ $result['event']['id'] }}" action="{{ route('events.approve', $result['event']['id']) }}" method="POST" style="display: none;">
                      @csrf
                      @method('PUT')
                    </form>
                    <form id="reject-form-{{ $result['event']['id'] }}" action="{{ route('events.reject', $result['event']['id']) }}" method="POST" style="display: none;">
                      @csrf
                      @method('PUT')
                    </form>

                    <a title="Approve" href="#" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm me-3 mb-3" onclick="event.preventDefault(); confirmApprove({{ $result['event']['id'] }});">
                      <i class="fas fa-check"></i>
                    </a>

                    <a title="Reject" href="#" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm mb-3" onclick="event.preventDefault(); confirmReject({{ $result['event']['id'] }});">
                      <i class="fas fa-times"></i>
                    </a>
                    <br/>
                    @endif
                    <a title="Show" href="{{ route($show, $result['event']['id']) }}" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm me-3">
                      <i class="fa-regular fa-eye"></i>
                    </a>
                    <a title="Edit" href="{{ route($edit, $result['event']['id']) }}" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm me-3">
                      <i class="fa-regular fa-pen-to-square"></i>
                    </a>
                    <!-- Delete Form (Hidden) -->
                    <form id="delete-form-{{ $result['event']['id'] }}" action="{{ route($destroy, $result['event']['id']) }}" method="POST" style="display: none;">
                      @csrf
                      @method('DELETE')
                    </form>

                    <!-- Delete Button -->
                    <a title="Delete" href="#" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm me-3" onclick="event.preventDefault(); confirmDelete({{ $result['event']['id'] }});">
                      <i class="fa-regular fa-trash-can"></i>
                    </a>
                    <br/>
                    @if($result['menu'] == null) 
                    <a title="Create Menu" href="{{ route('events.create-menu', $result['event']['id']) }}" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm mt-3 me-3">
                      <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" width="20" height="20" x="0" y="0" viewBox="0 0 64 64" style="enable-background:new 0 0 512 512" xml:space="preserve" class="hovered-paths"><g><path d="M15.158 38.419a1 1 0 0 0 .643 1.26c3.415 1.108 8.86 1.729 16.215 1.843 7.346-.114 12.796-.738 16.197-1.854a1 1 0 0 0-.623-1.9c-3.21 1.053-8.461 1.643-15.574 1.754-7.124-.111-12.372-.698-15.598-1.746a1 1 0 0 0-1.26.643z" fill="currentColor" opacity="1" data-original="currentColor" class="hovered-path"></path><path d="M52.23 35.188c-.766-10.4-11.003-14.7-19.226-14.523v-.884c1.78-1.332 2.337-1.925 2.898-3.11a2.7 2.7 0 0 0-3.848-3.216 2.688 2.688 0 0 0-3.698 3.691 9.98 9.98 0 0 0 2.587 2.69c.02.014.042.019.06.032v.798a24.561 24.561 0 0 0-7.099.944 1 1 0 0 0 .592 1.91c8.11-1.872 19.849-1.599 24.612 7.329.869 1.507.776 3.512 1.364 5.446a6.37 6.37 0 0 0 1.922 2.91c.97 2.025-3.212 3.132-5.228 3.636a1 1 0 0 0 .447 1.949c3.624-.708 8.696-3.028 6.39-6.775 12.021 5.603-5.658 11.027-21.999 10.94-16.358.087-34.02-5.336-22.004-10.943a3.083 3.083 0 0 0-.531 2.258c1.366 5.118 12.138 5.638 22.545 5.976a102.944 102.944 0 0 0 11.505-.73 1 1 0 0 0-.258-1.982 100.852 100.852 0 0 1-11.245.711c-12.303-.186-17.276-1.667-19.28-2.876-1.023-.603-1.617-1.365-1.133-2.163 2.746-2.042 1.992-6.466 3.634-8.993a13.518 13.518 0 0 1 5.671-5.299 1 1 0 0 0-.916-1.778 14.55 14.55 0 0 0-8.233 12.049C6.684 36.892 4 39.138 4 41.7c0 6.011 14.429 9.255 28.004 9.255C45.576 50.955 60 47.71 60 41.7c0-2.57-2.689-4.815-7.77-6.512zM30.057 16.094a.665.665 0 0 1 .601-1.039.697.697 0 0 1 .071.004.598.598 0 0 1 .412.227 1 1 0 0 0 1.738.042.593.593 0 0 1 .422-.208.672.672 0 0 1 .625 1.062 7.894 7.894 0 0 1-1.983 1.905 7.895 7.895 0 0 1-1.886-1.993z" fill="currentColor" opacity="1" data-original="currentColor" class="hovered-path"></path></g></svg>
                    </a>
                    <a title="Chef Input" href="{{ route('events.create-chef-menu', $result['event']['id']) }}" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm me-3 mt-3">
                    ...
                    @else
                    <a title="Menu Item List" href="{{ route('menu-items.show', $result['menu']['id']) }}" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm me-3 mt-3">
                      <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" width="20" height="20" x="0" y="0" viewBox="0 0 682.667 682.667" style="enable-background:new 0 0 512 512" xml:space="preserve" class=""><g><defs><clipPath id="a" clipPathUnits="userSpaceOnUse"><path d="M0 512h512V0H0Z" fill="currentColor" opacity="1" data-original="currentColor"></path></clipPath></defs><g clip-path="url(#a)" transform="matrix(1.33333 0 0 -1.33333 0 682.667)"><path d="m0 0 224.934 112.467V0Z" style="stroke-width:30;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:10;stroke-dasharray:none;stroke-opacity:1" transform="translate(111.4 384.533)" fill="none" stroke="currentColor" stroke-width="30" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" stroke-dasharray="none" stroke-opacity="" data-original="currentColor" class=""></path><path d="M111.4 15h289.2v369.533H111.4Z" style="stroke-width:30;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:10;stroke-dasharray:none;stroke-opacity:1" fill="none" stroke="currentColor" stroke-width="30" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" stroke-dasharray="none" stroke-opacity="" data-original="currentColor" class=""></path><path d="M0 0h-128.533c0 35.475 28.797 64.267 64.266 64.267C-28.797 64.267 0 35.475 0 0Z" style="stroke-width:30;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:10;stroke-dasharray:none;stroke-opacity:1" transform="translate(320.267 223.867)" fill="none" stroke="currentColor" stroke-width="30" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" stroke-dasharray="none" stroke-opacity="" data-original="currentColor" class=""></path><path d="M0 0v16.067" style="stroke-width:30;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:10;stroke-dasharray:none;stroke-opacity:1" transform="translate(256 288.133)" fill="none" stroke="currentColor" stroke-width="30" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" stroke-dasharray="none" stroke-opacity="" data-original="currentColor" class=""></path><path d="M0 0h160.667" style="stroke-width:30;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:10;stroke-dasharray:none;stroke-opacity:1" transform="translate(175.667 223.867)" fill="none" stroke="currentColor" stroke-width="30" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" stroke-dasharray="none" stroke-opacity="" data-original="currentColor" class=""></path><path d="M0 0h160.667" style="stroke-width:30;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:10;stroke-dasharray:none;stroke-opacity:1" transform="translate(175.667 143.533)" fill="none" stroke="currentColor" stroke-width="30" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" stroke-dasharray="none" stroke-opacity="" data-original="currentColor" class=""></path><path d="M0 0h64.267" style="stroke-width:30;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:10;stroke-dasharray:none;stroke-opacity:1" transform="translate(223.867 79.267)" fill="none" stroke="currentColor" stroke-width="30" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" stroke-dasharray="none" stroke-opacity="" data-original="currentColor" class=""></path></g></g></svg>
                    </a>
                    
                    @endif 

                    @if($result['menu'] != null && $result['menu']->item_quantity == 'chef-input') 
                      <a title="Edit Chef Input" href="{{ route('menus.edit-chef-input', $result['menu']->id) }}" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm me-3 mt-3">
                  ...
                  <td>
                    {{ $result['event']['created_by'] ? $result['event']['created_by']['name'] : '-' }}
                    <br />
                    {{ \Carbon\Carbon::parse($result['event']['created_at'])->isoFormat('Do MMM YYYY, hh:mm A') }}
                  </td>
                  <td>
                    {{ $result['event']['updated_by'] ? $result['event']['updated_by']['name'] : '-' }}
                    <br />
                    {{ $result['event']['updated_at'] ? \Carbon\Carbon::parse($result['event']['updated_at'])->isoFormat('Do MMM YYYY, hh:mm A') :'-' }}
                  </td>
                </tr>
                @endforeach
              </tbody>
...
<script>
  ...
  function confirmApprove(id) {
    if (confirm('Are you sure you want to approved this record?')) {
      var form = document.getElementById('approved-form-' + id);
      if (form) {
        form.submit();
      } else {
        console.error('Form not found for record ID:', id);
