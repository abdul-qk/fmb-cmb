                                ...
                                @foreach($roles as $key => $role)
                                    <tr>
                                        ...
                                        <td>{{ 
                                         ...
                                         <br/>
                                         {{ $role->created_at->isoFormat('Do MMM YYYY, hh:mm A') }}
                                        </td>
                                        <td>
                                          {{ $role->updatedBy ? $role->updatedBy->name : '-' }}
                                          <br/>
                                          {{ $role->updated_at ? $role->updated_at->isoFormat('Do MMM YYYY, hh:mm A') : "-" }}
                                         </td>
                                        <td>
                                            @if(hasPermissionForModule('edit', $currentModuleId))
                                            <a title="Edit" href="{{ route($edit, $role->id) }}" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm me-3">
                                              <i class="fa-regular fa-pen-to-square"></i>
                                            </a>
                                            @endif

                                            <!-- Delete Form (Hidden) -->
                                            <form id="delete-form-{{ $role->id }}" action="{{ route($destroy,$role->id) }}" method="POST" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                            ...
                                            @if(hasPermissionForModule('delete', $currentModuleId))
                                                <a title="Delete" href="#" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm me-3" onclick="event.preventDefault(); confirmDelete({{ $role->id }});">
