      ...
      <h2 class="border-bottom pb-2 border-bottom-3 border-primary mb-3">#{{$results->id}}</h2>
        <div class="row">
          ...
          <div class="col-md-3 col-lg-3 mb-5">
            ...
            <h5 class="border-primary" style=" word-break: break-word;height: 43px; border-bottom: 1px solid #ddd; display: flex; align-items: center; color: #4b5675; font-size: 1.1rem; ">
              {{ $results->created_at ? $results->created_at->isoFormat('Do MMM YYYY') : "-" }}
              <br />
              {{ $results->created_at ? $results->created_at->isoFormat('hh:mm A') : "-" }}
            </h5>
          </div>
            ...
            <div class="table-responsive">
              <table class="table table-bordered">
                ...
                <tbody id="items-table-body">
                  ...
                  <tr class="item-category-{{$data['item_category_id']}}" id="item-row-{{ $index }}">
                    <td style="vertical-align: middle;">{{$data['item_name']}}</td>
                    <td style="vertical-align: middle;">{{$data['return_uom']}}</td>
                    <td style="vertical-align: middle;">{{$data['return_uom']}}</td>
                    <td style="vertical-align: middle;">{{$data['returned_quantity']}}</td>
                  </tr>
                  @endforeach
