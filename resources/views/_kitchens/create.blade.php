        ...
        <form class="form" method="POST" action="{{ route($store) }}">
          ...
          <div class="row">
            <div class="col-md-4 col-lg-3 mb-5">
              ...
              <select class="form-select " name="location_id" data-control="select2" data-close-on-select="false" data-placeholder="Select" data-allow-clear="true">
                <option value=""></option>
                @foreach($locations as $location)
                  <option value="{{ $location->id }}" {{ old('location_id') == $location->id ? 'selected' : '' }}>{{ $location->area }}</option>
                @endforeach
              </select>
