            ...
            <table id="kt_datatable" class="table table-bordered gy-5">
              ...
              <tbody>
                ...
                <tr>
                  <td>{{ $result['place'] ?? '-' }}</td>
                  <td>
                    {{ \Carbon\Carbon::parse($result['date'])->isoFormat('Do MMM YYYY') }} <br />
                  ...
                  <td>{{ $result['event'] ?? '-' }}
                    <br/>
                    <br/>
                   <b> Item Selection:</b> {{ Str::title(str_replace('-', ' ', $result['item_quantity'])) }}
                  </td>
                  <td>
                    @foreach ($result['dishes'] as $dish)
                    {{ $dish['dish'] }} <br/>  {{ $dish['chef'] }}
                    <hr />
                    @endforeach
                  </td>
                  <td>
                    @foreach ($result['dishes'] as $dish)
                    @foreach ($dish['recipe_items'] as $recipeItem)
                    {{ $recipeItem['Item'] }} - {{ $recipeItem['Quantity'] }} {{ $recipeItem['UOM'] }}
                    <br />
                    @endforeach
                    <hr />
                    @endforeach
                  ...
                  <td>
                    ...
                    {{ $result['created'] ? \Carbon\Carbon::parse($result['created'])->isoFormat('Do MMM YYYY, hh:mm A') : "-" }}
                  </td>
                  <td>
                    {{ $result['last_modified_by'] ?? '-' }}<br />
                    {{ $result['last_modified'] ? \Carbon\Carbon::parse($result['last_modified'])->isoFormat('Do MMM YYYY, hh:mm A') : "-" }}
                  </td>
                </tr>
                @endforeach
              </tbody>
