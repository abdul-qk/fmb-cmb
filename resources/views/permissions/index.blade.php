                            ...
                            <tbody>
                                @foreach($permissions as $key => $permission)
                                    <tr>
                                        ...
                                        <td>{{
                                          $permission->createdBy ? $permission->createdBy->name : '-'
                                         }} 
                                         <br/>
                                         {{ $permission->created_at->isoFormat('Do MMM YYYY, hh:mm A') }}
                                        </td>
                                        <td>
                                          {{ $permission->updatedBy ? $permission->updatedBy->name : '-' }}
                                          <br/>
                                          {{ $permission->updated_at ? $permission->updated_at->isoFormat('Do MMM YYYY, hh:mm A') : "-" }}
                                         </td>
                                        <td>
                                            @if(hasPermissionForModule('edit', $currentModuleId))
                                            <a title="Edit" href="{{ route($edit, $permission->id) }}" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm me-3">
                                              <i class="fa-regular fa-pen-to-square"></i>
                                            </a>
                                            @endif

                                            <!-- Delete Form (Hidden) -->
                                            <form id="delete-form-{{ $permission->id }}" action="{{ route($destroy,$permission->id) }}" method="POST" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                            ...
                                            @if(hasPermissionForModule('delete', $currentModuleId))
                                                <a title="Delete" href="#" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm me-3" onclick="event.preventDefault(); confirmDelete({{ $permission->id }});">
