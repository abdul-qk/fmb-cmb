            ...
            <table id="kt_datatable" class="table table-bordered gy-5">
              ...
              <tbody>
                ...
                <tr class="text-capitalize">
                  ...
                  <td>{{ $result['date_from'] ? \Carbon\Carbon::parse($result['date_from'])->isoFormat('Do MMM YYYY') :'-' }}</td>
                  <td>{{ $result['date_to'] ? \Carbon\Carbon::parse($result['date_to'])->isoFormat('Do MMM YYYY') :'-' }}</td>
                  <td>
                    {{ $result['quantity'] }}
                  </td>
                  @else
                  <td>
                    ...
                    <div>
                      Tiffin {{ $item['servingQuantityTiffinItems']['name'] ?? 'N/A' }}
                      ({{ $item['servingQuantityTiffinItems']['person_no'] ?? 'N/A' }} pax)
                    </div>
                    @if(!$loop->last)
                    <hr>
                    @endif
                  ...
                  <td>
                    @foreach($result['servingQuantityItems'] as $index => $item)
                    <div>
                      {{ $item['date_from'] ? \Carbon\Carbon::parse($item['date_from'])->isoFormat('Do MMM YYYY') :'-' }}
                    </div>
                    @if(!$loop->last)
                    <hr>
                    @endif
                  ...
                  <td>
                    @foreach($result['servingQuantityItems'] as $index => $item)
                    <div>
                     {{ $item['date_to'] ? \Carbon\Carbon::parse($item['date_to'])->isoFormat('Do MMM YYYY') :'-' }}
                    </div>
                    @if(!$loop->last)
                    <hr>
                    @endif
