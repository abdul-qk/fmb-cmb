        ...
        <form class="form" method="POST" action="{{ route($update, $result->id) }}">
          ...
          <div class="row">
            ...
            <div class="col-md-4 col-lg-3 mb-5 thaalField" style="{{ old('serving', $result['serving']) == 'Thaal' ? '' : 'display: none;' }}">
              ...
              <input type="number" class="form-control" min="0" id="quantity" name="quantity" value="{{ old('quantity', $result['quantity']) }}">
            ...
            <div class="col-md-4 col-lg-3 mb-5 thaalField" style="{{ old('serving', $result['serving']) == 'Thaal' ? '' : 'display: none;' }}">
              ...
              <input type="date" class="form-control" name="date_from" id="date_from" value="{{ old('date_from', $result['date_from']) }}">
            ...
            <div class="col-md-4 col-lg-3 mb-5 thaalField" style="{{ old('serving', $result['serving']) == 'Thaal' ? '' : 'display: none;' }}">
              ...
              <input type="date" class="form-control" name="date_to" id="date_to" value="{{ old('date_to', $result['date_to']) }}">
          ...
          <div class="row">
            <div id="tiffinField" style="{{ old('serving', $result['serving']) == 'Tiffin' ? '' : 'display: none;' }}" class="col-md-12 mb-5">
              ...
              <div class="tiffin-group">
                ...
                <input hidden type="text" min="0" value="{{ old('items.person_no.' . $item->id, $item['servingQuantityTiffinItems']['person_no']) }}" name="items[{{ $index }}][person_no]" />
                ...
                <div class="row">
                  <div class="col-md-4 col-lg-3 mb-5">
                    ...
                    <input type="number" min="0" value="{{ old('items.' . $index . '.quantity', $item['quantity']) }}" name="items[{{ $index }}][quantity]" id="quantity_{{ $item['id'] }}" placeholder="Quantity" class="form-control" />
                  </div>

                  <div class="col-md-4 col-lg-3 mb-5">
                    <label class="form-label required">Date From</label>
                    <input type="date" class="form-control" name="items[{{ $index }}][date_from]" id="date_from_{{ $item['id'] }}" value="{{ old('items.' . $index . '.date_from', $item['date_from']) }}">
                    @if ($errors->has('items.' . $index . '.date_from'))
                    <span class="text-danger">{{ $errors->first('items.' . $index . '.date_from') }}</span>
                    @endif
                  </div>

                  <div class="col-md-4 col-lg-3 mb-5">
                    <label class="form-label required">Date To</label>
                    <input type="date" class="form-control" name="items[{{ $index }}][date_to]" id="date_to_{{ $item['id'] }}" value="{{ old('items.' . $index . '.date_to', $item['date_to']) }}">
                    @if ($errors->has('items.' . $index . '.date_to'))
                    <span class="text-danger">{{ $errors->first('items.' . $index . '.date_to') }}</span>
                    @endif
                  </div>
                </div>
              </div>
