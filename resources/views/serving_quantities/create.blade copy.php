        ...
        <form class="form" method="POST" action="{{ route($store) }}">
          ...
          <div class="row">
            <div class="col-md-3 mb-5">
              ...
              <select class="form-select" id="serving" name="serving" data-control="select2" data-close-on-select="false" data-placeholder="Select" data-allow-clear="true">
                ...
                <option value="Thaal" {{ old('serving',Thaal) == 'Thaal' ? 'selected' : '' }}>Thaal</option>
              </select>
            ...
            <div class="col-md-3 mb-5 thaalField">
          ...
          <div class="row">
            <div id="tiffanField" style="display: none;" class="col-md-12 mb-5">
              @foreach($results as $index => $item) <!-- Changed variable name from result to item -->
              <div class="tiffin-group">
                <h5 class="text-capitalize">Tiffin ({{ $item->name }} <span>{{ $item->person_no }}</span> pax{{ $item->person_no == 1 ? "" : "s" }})</h5>

                <input type="text" min="0" value="{{ old('items.tiffin_size_id.' . $item->id, $item->id) }}" name="items[{{ $index }}][tiffin_size_id]" id="tiffin_size_id_{{ $item->id }}" placeholder="No. of Tiffin" class="form-control tiffin_size_id" />
                <hr />

                <div class="row mb-3">
                  <div class="col-sm-2">
                    <label class="form-label text-capitalize required">Quantity</label>
                    <input type="number" min="0" value="{{ old('items.quantity.' . $item->id, $item->quantity) }}" name="items[{{ $index }}][quantity]" id="quantity_{{ $item->id }}" placeholder="Quantity" class="form-control" />
                  </div>

                  <div class="col-md-3 mb-5">
                    <label class="form-label required">Date From</label>
                    <input type="date" class="form-control" name="items[{{ $index }}][date_from]" id="date_from_{{ $item->id }}" value="{{ old('items.date_from.' . $item->id) }}">
                  ...
                  <div class="col-md-3 mb-5">
                    <label class="form-label required">Date To</label>
                    <input type="date" class="form-control" name="items[{{ $index }}][date_to]" id="date_to_{{ $item->id }}" value="{{ old('items.date_to.' . $item->id) }}">
