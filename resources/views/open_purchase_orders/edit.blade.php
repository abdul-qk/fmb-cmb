              ...
              <table class="table table-bordered">
                <thead>
                  <tr>
                    ...
                    <th class="">Qty</th>
                    <th class="">Unit</th>
                    <th class="">Per Unit Price</th>
                    <th class="">Total</th>
                    <th title="Add More" class="text-center"> 
                      <svg  id="add-item-btn" xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" width="25" height="25" x="0" y="0" viewBox="0 0 24 24" style="enable-background:new 0 0 512 512" xml:space="preserve" class="cursor-pointer"><g><path d="M18 2c2.206 0 4 1.794 4 4v12c0 2.206-1.794 4-4 4H6c-2.206 0-4-1.794-4-4V6c0-2.206 1.794-4 4-4zm0-2H6a6 6 0 0 0-6 6v12a6 6 0 0 0 6 6h12a6 6 0 0 0 6-6V6a6 6 0 0 0-6-6z" fill="var(--bs-white)" opacity="1" data-original="#000000" class=""></path><path d="M12 18a1 1 0 0 1-1-1V7a1 1 0 0 1 2 0v10a1 1 0 0 1-1 1z" fill="var(--bs-white)" opacity="1" data-original="#000000" class=""></path><path d="M6 12a1 1 0 0 1 1-1h10a1 1 0 0 1 0 2H7a1 1 0 0 1-1-1z" fill="var(--bs-white)" opacity="1" data-original="#000000" class=""></path></g></svg>
                    </th>
                  </tr>
                </thead>
                <tbody id="items-table-body">
                  ...
                  <tr id="item-row-{{ $index }}">
                      <td>
                          <select class="form-select item-select" name="items[{{ $index }}][item_id]" data-control="select2" data-close-on-select="false" data-placeholder="Select" data-allow-clear="true">
                              ...
                              <option value="{{ $availableItem->id }}" 
                                {{ (old('items.' . $index . '.item_id') == $availableItem->id) || (isset($item['item']['id']) && $item['item']['id'] == $availableItem->id) ? 'selected' : '' }}>
                                {{ $availableItem->name }}
                              </option>
                              @endforeach
                          </select>
                      ...
                      <td>
                          <input type="number" step="0.001" min="0" class="form-control quantity-input w-150px w-lg-100" name="items[{{ $index }}][quantity]" value="{{ $item['quantity'] ?? '' }}">
                      </td>
                      <td>
                        <div class="position-relative">
                          ...
                          <select class="form-select unit-select" data-id="{{ isset($item['unit_measure']['id']) ? $item['unit_measure']['id'] :''  }}" name="items[{{ $index }}][unit_id]" data-control="select2" data-close-on-select="false" data-placeholder="Select" data-allow-clear="true">
                              <option value=""></option>
                              <!-- @foreach($units as $unit)
                              <option value="{{ $unit->id }}" 
                                {{ (old('items.' . $index . '.unit_id') == $unit->id) || (isset($item['unit_measure']['id']) && $item['unit_measure']['id'] == $unit->id) ? 'selected' : '' }}>
                                {{ $unit->name }}
                              </option>
                              @endforeach -->
                          </select>
                      ...
                      <td>
                          <input type="number" class="form-control item-total w-150px w-lg-100" value="{{ ($item['quantity'] ?? 0) * ($item['unit_price'] ?? 0) }}" disabled>
...
<script>
  $(document).ready(function() {
    let itemIndex = {{ count(old('items', $result->detail->toArray())) }};
    let fetchedItems = @json($items);
    let fetchedUnits = @json($units);
        ...
        <tr id="item-row-${itemIndex}">
          ...
          <td>
            <div class="position-relative">
              ...
              <select class="form-select unit-select" name="items[${itemIndex}][unit_id]">
                <option value=""></option>
              </select>
            </div>
          </td>
          <td>
            <input type="number" step="0.01" class="form-control unit-price-input w-150px w-lg-100" name="items[${itemIndex}][unit_price]">
          </td>
          <td>
    ...
    $(document).on('change','.item-select', function() {
      ...
      $.get("/fetch-unit", data, function(response) {
        response?.result?.forEach(function(item) {
          $(unit_measure).select2().append(new Option(item.short_form, item.id, current_unit_measure ==  item.id, current_unit_measure ==  item.id)).trigger('change');
        });
