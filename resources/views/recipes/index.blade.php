          ...
          <div class="table-responsive">
            <table id="kt_datatable" class="table table-bordered gy-5">
              ...
              <tbody>
                ...
                <tr>
                  <td>{!! $result->place['name'] .'<br/> '. $result->place['location']['area'] !!}</td>
                  <!-- <td>{{$result->chefUser->name}}</td> -->
                 
                  <td>{{$result->dish['name']}}</td>
                  <td>{{$result->dish->dishCategory->name}}</td>
                  <td class="text-capitalize">{{$result->serving_item}}</td>
                  <td>{{$result->serving}}</td>
                  <td>
                  ...
                  @if(hasPermissionForModule('show', $currentModuleId))
                    <a title="Show" href="{{ route($show, ['recipe' => $result->id]) }}" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm me-3">
                      <i class="fa-regular fa-eye"></i>
                    </a>
                    @endif
                    @if(hasPermissionForModule('edit', $currentModuleId))
                    <a title="Edit" href="{{ route($edit, ['recipe' => $result->id]) }}" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm me-3">
                      <i class="fa-regular fa-pen-to-square"></i>
                    </a>
                    @endif
                    @if(hasPermissionForModule('delete', $currentModuleId))
                    <!-- Delete Form (Hidden) -->
                    <form id="delete-form-{{ $result->id }}" action="{{ route($destroy, ['recipe' => $result->id]) }}" method="POST" style="display: none;">
                      @csrf
                      @method('DELETE')
                    </form>
  ...
  function confirmDelete(id) {
    if (confirm('Are you sure you want to delete this Recipe?')) {
      ...
      if (form) {
        ...
        console.error('Form not found for Recipe ID:', id);
      }
    }
  }
</script>
