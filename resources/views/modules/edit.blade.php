        ...
        <!--begin::Container-->
         <div class="container-fluid" id="kt_content_container">
            ...
            <div class="bg-transparent border-0 card shadow-none pt-2">
                <form class="form" method="POST" action="{{ route($update, ['module' => $editableModule->id]) }}">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-4 col-lg-3 mb-5">
                          ...
                          <select class="form-select form-select-solid email-block" id="parent_id" name="parent_id" data-control="select2" data-close-on-select="false" data-placeholder="Select" data-allow-clear="true">
                            ...
                            @foreach($modules as $module)
                              <option value="{{ $module->id }}" {{ old('parent_id', $editableModule->parent_id) == $module->id ? 'selected' : '' }}>{{ $module->name }}</option>
                            @endforeach
                          </select>
                        ...
                        <div class="col-md-4 col-lg-3 mb-5">
                          ...
                          <input type="text" class="form-control" name="name" id="name" value="{{old('name',$editableModule->name)}}">
                          @if ($errors->has('name'))
                        ...
                        <input type="text" class="form-control" name="icon" id="icon" value="{{old('icon',$editableModule->icon)}}">
                        @if ($errors->has('icon'))
                        ...
                        <input type="number" class="form-control" name="display_order" id="display_order" value="{{old('display_order',$editableModule->display_order)}}">
...
@push('scripts')
  ...
  $(document).ready(function() {
    $('#parent_id').change(function() {
      ...
      var current_module_id = "{{ $editableModule->id }}";
      $.ajax({
        type: 'GET',
        url: "{{ route('fetchModuleDisplayOrder') }}",
        data: { parent_id: parent_id, current_module_id: current_module_id },
