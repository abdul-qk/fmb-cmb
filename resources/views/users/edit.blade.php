        ...
        <!--begin::Container-->
         <div class="container-fluid" id="kt_content_container">
            ...
            <div class="bg-transparent border-0 card shadow-none pt-2">
                <form class="form" method="POST" action="{{ route($update, $user->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        ...
                        <div class="col-md-4 col-lg-3 mb-5">
                          ...
                          <select class="form-select " name="place_id" data-control="select2" data-close-on-select="false" data-placeholder="Select" data-allow-clear="true">
                            ...
                            @foreach($places as $place)
                              <option value="{{ $place->id }}" {{ old('place_id',$user->place_id) == $place->id ? 'selected' : '' }}>{{ $place->name }}</option>
                            @endforeach
                          </select>
                        ...
                        <div class="col-md-4 col-lg-3 mb-5">
                          ...
                          <select class="form-select form-select-solid email-block" name="role" data-control="select2" data-close-on-select="false" data-placeholder="Select" data-allow-clear="true">
                            <option value=""></option>
                            @foreach($roles as $role)
                              <option value="{{ $role->name }}" {{ old('role', $userRole?->name) == $role->name ? 'selected' : '' }}>{{ $role->name }}</option>
                            @endforeach
                          </select>
                          @if ($errors->has('role'))
