  ...
  <div id="kt_post">
    ...
    <div class="container-fluid" id="kt_content_container">
      ...
      <div class="bg-transparent border-0 card shadow-none pt-2">
        <form class="form" method="POST" action="{{ route($update, ['item' => $result->id]) }}">
          @csrf
          @method('PUT')
          <div class="row">
            <div class="col-md-4 col-lg-3 mb-5">
              ...
              <select class="form-select " name="category_id" data-control="select2" data-close-on-select="false" data-placeholder="Select" data-allow-clear="true">
                ...
                <option value="{{ $itemsCategory->id }}" {{ old('category_id', $result->category_id) == $itemsCategory->id ? 'selected' : '' }}>{{ $itemsCategory->name }}</option>
                @endforeach
              </select>
            ...
            <div class="col-md-4 col-lg-3 mb-5">
              ...
              <input type="hidden" class="form-control" name="base_uom_id" id="base_uom_id" value="{{old('name',$itemBaseUom->id)}}">
              <select class="form-select " id="unit_measure_id" name="unit_measure_id" data-control="select2" data-close-on-select="false" data-placeholder="Select" data-allow-clear="true">
                <option value=""></option>
                @foreach($unitMeasure as $unitItem)
                <option value="{{ $unitItem->id }}" {{ old('unit_measure_id',$itemBaseUom->unit_measure_id) == $unitItem->id ? 'selected' : '' }}>{{ $unitItem->name }}</option>
                @endforeach
              </select>
...
@php
    $unitMeasureIds = $itemBaseUom ? $itemBaseUom->unitMeasure->pluck('id')->toArray() : [];
@endphp

@push('scripts')
<script>
  let anyError = @json($errors->any());
  console.log(@json($errors->all()));

  let unitMeasureIds = @json($unitMeasureIds);
  console.log(unitMeasureIds);

  if (anyError) {
    $('#unit_measure_id').trigger("change");
  }
  ...
  $(document).on("change", "#unit_measure_id", function() {
    ...
    @foreach($unitMeasure as $result)
      if (baseUomSelect != "{{ $result->id }}") {
        options += `<option value="{{ $result->id }}" ${unitMeasureIds.includes({{ $result->id }}) || baseUomSelect == "{{ $result->id }}" ? 'selected' : ''}>{{ $result->name }}</option>`;
      }
    @endforeach
