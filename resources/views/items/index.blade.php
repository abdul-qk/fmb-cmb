          ...
          <div class="table-responsive">
            <table id="kt_datatable" class="table table-bordered gy-5">
              ...
              <tbody>
                ...
                <tr>
                  <td>{{$result->itemCategory['name']}}</td>
                  <td>{{$result->name}}</td>
                  <td>{{ $result->itemBase ? $result->itemBase->baseUom->name ?? '-' : '-' }}</td>
                  <td>{{ $result->itemBase ? $result->itemBase->unitMeasure->pluck('name')->implode(', ') ?? '-' : '-' }}</td>
                  
                  <td>
                    @if(hasPermissionForModule('edit', $currentModuleId))
                      <a title="Edit" href="{{ route($edit, ['item' => $result->id]) }}" class="btn btn-icon btn-color-muted btn-bg-light btn-active-color-primary btn-sm me-3">
                        <i class="fa-regular fa-pen-to-square"></i>
                      </a>
                    @endif
                    @if(hasPermissionForModule('delete', $currentModuleId))
                    <!-- Delete Form (Hidden) -->
                    <form id="delete-form-{{ $result->id }}" action="{{ route($destroy, ['item' => $result->id]) }}" method="POST" style="display: none;">
                      @csrf
                      @method('DELETE')
                    </form>
